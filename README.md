...
| **6-7 — 写作润色** | `Meng-Skills` | 中文论文与专业文本写作/润色总技能：写前清单、实证章节写法规范、去 AI 味与降 AIGC 检测（阶段 6 写作、阶段 7 打磨流水线的机械润色/结构调整/二次润色均调用） | [链接](https://github.com/MartyYao/meng-skills) |
...
## v0.4.2 更新说明（2026-08-13）

- 依赖自检安装方式改为从 `paper-workflow-bundle` 下载 zip 拷贝；弃用未认证的 `hermes skills install`，避免 GitHub API 限额和 registry 同名技能劫持。
- 安装后继续执行版本检查，确保加载的是本全家桶版本而不是 registry 同名技能。

## v0.4.1 更新说明（2026-08-13）

- 阶段 5→6 准入审查新增数字对账：正文表格数字必须与 CSV 一致。
- 强化 `rerun.sh new "<tag>"` 作为新实证 run 的唯一入口，修复 `esttab2pipe.py` 幽灵引用。

## v0.4.0 更新说明（2026-08-13）
